...
bool _init( const int argc, char** argv )
{
	...
	/*	MPI library set by default the MPI_THREAD_SINGLE level of thread support.
		Collage is a multithreaded library, so, the required level of thread 
		support should be MPI_THREAD_SERIALIZED at least.

		NOTE:
		Be aware that MPI_THREAD_MULTIPLE is only lightly tested and likely still
		has some bugs. Please, refer the below links:
